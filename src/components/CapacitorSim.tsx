    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6 space-y-6 text-slate-100 shadow-xl">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-white">{tI18n('experiments.capacitor_lab.title')}</h2>
        ...
        >
          ...
          {tI18n('experiments.capacitor_lab.reset')}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between items-center relative min-h-[340px]">
          ...
          <div className="w-full flex flex-col items-center pt-2">
            ...
            <span className="text-[10px] text-red-400 font-semibold mt-0.5">{tI18n('experiments.capacitor_lab.topPlateLabel')}</span>
          </div>
          ...
          <div className="w-full flex flex-col items-center pb-2">
            <span className="text-[10px] text-blue-400 font-semibold mb-0.5">{tI18n('experiments.capacitor_lab.bottomPlateLabel')}</span>
            <div
              style={{ width: `${Math.min(Math.max((plateArea_mm2 / 400) * 100, 30), 90)}%` }}
          ...
          <div className="w-full grid grid-cols-3 gap-2 border-t border-slate-800 pt-3 text-center">
            <div className="p-2 bg-slate-900 rounded-lg">
              <span className="text-[10px] text-slate-400 block">{tI18n('experiments.capacitor_lab.capacitance')}</span>
              <span className="text-sm font-bold text-sky-400 font-mono">{capacitance_pF.toFixed(2)} pF</span>
            </div>
            <div className="p-2 bg-slate-900 rounded-lg">
              <span className="text-[10px] text-slate-400 block">{tI18n('experiments.capacitor_lab.storedCharge')}</span>
              <span className="text-sm font-bold text-amber-400 font-mono">{charge_pC.toFixed(2)} pC</span>
            </div>
            <div className="p-2 bg-slate-900 rounded-lg">
              <span className="text-[10px] text-slate-400 block">{tI18n('experiments.capacitor_lab.storedEnergy')}</span>
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-amber-400">{tI18n('experiments.capacitor_lab.batteryVoltage')} (V)</span>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-slate-300">{tI18n('experiments.capacitor_lab.plateArea')} (A)</span>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-slate-300">{tI18n('experiments.capacitor_lab.separation')} (d)</span>
            ...
            <div className="space-y-1.5 pt-1">
              <label className="text-xs font-semibold text-slate-300 block">{tI18n('experiments.capacitor_lab.dielectric')}</label>
