  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (activeView === 'spectrometer') {
        ...
        ctx.fillText(tI18n('experiments.atomic_spectra.gasDischarge'), tubeX - 10, tubeY + tubeH + 24);
        ...
        ctx.font = '10px sans-serif';
        ctx.fillText(tI18n('experiments.atomic_spectra.grating'), gratingX - 8, tubeY + 115);
        ...
        ctx.fillText(tI18n('experiments.atomic_spectra.wavelengthRuler'), specX + specW / 2, rulerY + 30);
      } else {
    ...
    <div id="atomic-spectra-simulation" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-zinc-800/80 mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{activeView === 'spectrometer' ? tI18n('experiments.atomic_spectra.bohrModel') : tI18n('experiments.atomic_spectra.spectrometer')}</span>
              </button>
            </div>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-pink-950/20 border border-pink-800/30 text-xs text-zinc-300 space-y-2">
          ...
          <p>
            {tI18n('experiments.atomic_spectra.theoryDesc')}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-5 shadow-xl space-y-5">
          <h4 className="text-sm font-bold text-zinc-200 pb-2 border-b border-zinc-800">
            {tI18n('experiments.atomic_spectra.chemElementTitle')}
          </h4>

          {/* Element Selection Buttons */}
          <div className="space-y-1.5">
            <label className="text-sm text-zinc-400">{tI18n('experiments.atomic_spectra.selectElementLabel')}</label>
          ...
          <div className="space-y-1.5 pt-2 border-t border-zinc-800">
            <label className="text-sm text-zinc-400">{tI18n('experiments.atomic_spectra.emissionLinesLabel')}</label>
          ...
          {selectedElementId === 'hydrogen' && (
            <div className="space-y-1.5 pt-2 border-t border-zinc-800">
              <label className="text-sm text-zinc-400">{tI18n('experiments.atomic_spectra.hydrogenTransitionLabel')}</label>
          ...
          <button className={`min-h-[44px] min-w-[44px] min-h-[44px] min-w-[44px] w-full py-3 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg transition-all ${ logged ? 'bg-emerald-600 text-white shadow-emerald-900/40' : 'bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 shadow-pink-900/30' }`}>
            ...
            <span>
              {logged ? tI18n('experiments.atomic_spectra.loggedSuccess') : tI18n('experiments.atomic_spectra.logBtn')}
            </span>
          </button>
        </div>
      </div>
